...
const solution: any = (input: string) => {
    ...
    const beacons = getCommonBeacons(alignedScanners)
    ...
    public calcRelativeVectors() {
        ...
        for (let i = 0; i < this.beacons.length - 1; i++) {
            ...
            for (let j = i + 1; j < this.beacons.length; j++) {
                this.vectors.push(getVector(b1, this.beacons[j]))
            }
        }
    }
...
function getCommonBeacons(scanners: Scanner[]): Point[] {
    const beacons = scanners.map(s => s.beacons).flat()
    const uniqueBeacons = new Map<string, Point>()
    beacons.forEach(point => {
        if (!uniqueBeacons.get(JSON.stringify(point))) {
            uniqueBeacons.set(JSON.stringify(point), point)
        }
    })
    return [...uniqueBeacons.values()]
}
...
function match(s1: Scanner, s2: Scanner): void {
    ...
    for (const intersection of intersections) {
        ...
        if (s1b && s2b) {
            const offset = getVector(s1b, s2b)
    ...
    function findFirstIntersectedBacon(intersection: Vector, s: Scanner) {
        for (let i = 0; i < s.beacons.length - 1; i++) {
            ...
            for (let j = i + 1; j < s.beacons.length; j++) {
                if (getVector(b, s.beacons[j]).isEqual(intersection)) {
                    return b;
                }
            }
        }
