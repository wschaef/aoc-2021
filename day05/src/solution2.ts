...
function draw(line: number[][], board: number[][]) {
    ...
    if (x1 == x2) {
        ...
        points = _.range(Math.abs(x2 - x1) + 1)
        const xOperation = x1 < x2 ? (a: number, b: number) => a + b : (a: number, b: number) => a - b
        const yOperation = y1 < y2 ? (a: number, b: number) => a + b : (a: number, b: number) => a - b
        points.forEach(p => board[xOperation(x1, p)][yOperation(y1, p)] += 1)
    }
}
